...
class Configuration:
    ...
    def __init__(self, conf_file, user_path=None, exit_after_create=True):
        self.__sections = [item for item in self.__class__.__dict__.values() if inspect.isclass(item) and issubclass(item, Section)]

        for section in self.__sections:
            self.__setattr__(section.__name__, section(self.__setKey))
        ...
        if not os.path.isfile(os.path.join(self.__conf_path, self.__conf_file)):
            ...
            for section in self.__sections:
                self.__parser[section.__name__] = self.__classToDict(section)
